...
/datum/reagent/consumable/ethanol/beer/emberwine/on_mob_end_metabolize(mob/living/carbon/human/C)
	C.remove_status_effect(/datum/status_effect/debuff/emberwine)
	var/datum/component/arousal/arousal = C.GetComponent(/datum/component/arousal)
	if(arousal)
		arousal.freeze_arousal(src, FALSE)
	..()

/datum/reagent/consumable/ethanol/beer/emberwine/on_mob_life(mob/living/carbon/human/C)
	. = ..() // base: increments current_cycle and metabolizes
	...
	if(cycle == 20)
		to_chat(C, span_notice("You feel a warm glow spreading through your stomach."))
		return .

	if(cycle >= 21 && cycle <= 25)
		if(arousal)
			arousal.adjust_arousal(src, 5)
		return .

	C.apply_status_effect(/datum/status_effect/debuff/emberwine)
	if(arousal)
		if(arousal.arousal_frozen)
			arousal.freeze_arousal(src, FALSE)
		if(arousal.arousal < 51)
			arousal.set_arousal(src, 55)
		arousal.adjust_charge(8)
		arousal.adjust_arousal(src, 1)

	if(prob(8))
		C.emote("moan", forced = TRUE)
		to_chat(C, span_love(high_message))
		if(istype(C.wear_armor, /obj/item/clothing))
			...
			switch(CL.armor_class)
				if(3)
					C.Immobilize(30)
					C.set_blurriness(5)
					to_chat(C, span_warning("Your armor chafes uncomfortably against your skin and makes it difficult to breathe."))
				if(2)
					C.Immobilize(15)
					C.set_blurriness(2)
					to_chat(C, span_warning("Your armor chafes uncomfortably against your skin."))
...
/datum/reagent/consumable/ethanol/beer/emberwine/overdose_start(mob/living/carbon/human/C)
	if(current_cycle < 20)
		to_chat(C, span_notice("You feel a warm glow spreading through your stomach."))
		sleep(10)
	to_chat(C, span_warning("The glow in your stomach spreads, rushing to your head and warming your face."))
	metabolization_rate = 0.2 * REAGENTS_METABOLISM
	C.emote("moan", forced = TRUE)

	var/datum/component/arousal/arousal = C.GetComponent(/datum/component/arousal)
	if(arousal)
		arousal.adjust_arousal(src, 2)
		arousal.adjust_charge(5)
...
/datum/reagent/consumable/ethanol/beer/emberwine/addiction_act_stage4(mob/living/carbon/human/C)
	...
	SEND_SIGNAL(C, COMSIG_SEX_ADJUST_AROUSAL, 1.5)
	return
